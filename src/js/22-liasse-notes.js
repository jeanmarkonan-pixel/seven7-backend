...
function liasseAccountDetailRows(ex, refs, which){
    ...
    for(var i=0;i<rows.length;i++){
        var m = liasseFindRef(rows[i].compte);
...
function liasseRenderMvtAmortBlock(block, ex){
    ...
    var rows = block.refs.map(function(ref){
        var line = mapByRef[ref] || {label:ref};
        var ouverture = liasseSumByRef(ex, ref, 'amort', 'SC-SD', 'opening');
        var dotations = liasseSumMovementByRef(ex, [ref], 'mc');
        var reprises = liasseSumMovementByRef(ex, [ref], 'md');
        var cloture = liasseSumByRef(ex, ref, 'amort', 'SC-SD', 'closing');
    ...
    return '<div class="liasse-note-block"><h4>'+block.title+'</h4>'+
        '<table class="liasse-table"><thead><tr><th>REF</th><th>LIBELLE</th>'+
        '<th>AMORT. OUVERTURE</th><th>DOTATIONS EXERCICE</th><th>REPRISES / SORTIES</th><th>AMORT. CLOTURE</th></tr></thead>'+
        '<tbody>'+rows+foot+'</tbody></table></div>';
}
...
function liasseRenderNote(num){
    ...
    var body = '';
    if(cfg.type === 'auto-detail'){
        body = cfg.blocks.map(liasseRenderDetailBlock).join('');
    } else if(cfg.type === 'auto-mvt'){
        body = cfg.blocks.map(function(b){ return liasseRenderMvtBlock(b); }).join('');
    } else if(cfg.type === 'auto-mvt-amort'){
        body = cfg.blocks.map(function(b){ return liasseRenderMvtAmortBlock(b); }).join('');
