...
// Un relevé bancaire réel commence souvent par plusieurs lignes de titre
// (nom de la banque, n° de compte, période) AVANT la vraie ligne d'en-tête
// — ex. Banque Atlantique : 4 lignes puis « Date de l'opération | Date
// Valeur | Référence | Libellé | Montant | Solde ». On cherche donc, dans
// les premières lignes, celle qui donne un en-tête exploitable (colonne
// Date + soit Débit/Crédit, soit Montant), plutôt que de supposer que
// c'est forcément la ligne 0.
function rbEnteteExploitable(mappe){
    return mappe.date !== undefined
        && ((mappe.debit !== undefined && mappe.credit !== undefined) || mappe.montant !== undefined);
}
function rbTrouverLigneEntete(lignesCellules){
    var limite = Math.min(15, lignesCellules.length);
    for(var i = 0; i < limite; i++){
        var cells = (lignesCellules[i] || []).map(function(c){ return String(c === undefined || c === null ? '' : c); });
        if(cells.filter(function(s){ return s.trim() !== ''; }).length < 2) continue;
        if(rbEnteteExploitable(secMapperEntetes(SEC_CHAMPS_RELEVE, cells))) return i;
    }
    return -1;
}
// lignesCellules : array de array-de-chaînes. Retourne { lignes, rejets,
// erreurEntete }. erreurEntete=true si aucune ligne d'en-tête exploitable
// n'est trouvée dans les 15 premières lignes — on refuse alors tout net
// plutôt que de deviner un mauvais ordre de colonnes (une correspondance
// sur de mauvaises colonnes serait une erreur silencieuse).
function rbParserLignesCellules(lignesCellules){
    ...
    var idxEntete = rbTrouverLigneEntete(lignesCellules);
    if(idxEntete === -1) return { lignes:[], rejets:[], erreurEntete:true };
    var entetes = (lignesCellules[idxEntete] || []).map(function(c){ return String(c === undefined || c === null ? '' : c); });
    ...
    var lignes = [], rejets = [];
    for(var i = idxEntete + 1; i < lignesCellules.length; i++){
...
// Le séparateur est cherché sur la MEILLEURE ligne des 15 premières (celle
// qui produit le plus de colonnes), pas sur la ligne 0 : un relevé collé
// depuis Excel commence souvent par des lignes de titre sans séparateur
// (« BANQUE ATLANTIQUE - Relevé de Compte »), qui feraient choisir un
// mauvais délimiteur à secDetecterDelimiteur.
function rbMeilleurDelimiteur(lignes){
    var candidats = ['\t', ';', ',', '|'];
    var meilleur = ';', maxCols = 1;
    lignes.slice(0, 15).forEach(function(l){
        candidats.forEach(function(d){
            var n = l.split(d).length;
            if(n > maxCols){ maxCols = n; meilleur = d; }
        });
    });
    return meilleur;
}
function rbImporterColle(){
    ...
    var delim = rbMeilleurDelimiteur(brut);
