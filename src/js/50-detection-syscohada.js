...
var SYSC_MOTS_CLES = [
    { motif: /achat.*(march|marchandise)/i, racine:'601' },
    { motif: /achat.*(mati[eè]re|premi[eè]re)/i, racine:'602' },
    { motif: /vente.*(march|marchandise)/i, racine:'701' },
    { motif: /(salaire|personnel|r[eé]mun[eé]ration)/i, racine:'66' },
    { motif: /banque/i, racine:'52' },
    { motif: /caisse/i, racine:'57' },
    { motif: /fournisseur/i, racine:'40' },
    { motif: /client/i, racine:'41' },
    { motif: /terrain/i, racine:'22' },
    { motif: /b[aâ]timent/i, racine:'23' },
    { motif: /(mat[eé]riel|mobilier)/i, racine:'24' },
    { motif: /^capital/i, racine:'101' }
];
    ...
    (balanceData.n || []).forEach(function(r){
        ...
        for(var i = 0; i < SYSC_MOTS_CLES.length; i++){
            ...
            out.push({ compte:compte, intitule:r.intitule,
                ...
                compteRecommande: regle.racine, montant: montant });
...
function syscDetecterGenerique(){
    ...
    (balanceData.n || []).forEach(function(r){
        ...
        out.push({ compte:compte, intitule:r.intitule,
            ...
            compteRecommande: compte.slice(0, 3) + '00X (un sous-compte par tiers)', montant: montant });
    });
    return out;
}
...
function syscDetecterIncoherenceNN1(){
    ...
    (balanceData.n || []).forEach(function(r){
        ...
        out.push({ compte:compteN, intitule:r.intitule,
            ...
            compteRecommande: compteN1, montant: (r.sd || 0) || (r.sc || 0) });
    });
    return out;
}
        ...
        rows.forEach(function(r){
            ...
            if(compte.slice(0, 2) !== racineAttendue){
                out.push({ compte:compte, intitule:r.intitule,
                    ...
                    compteRecommande: racineAttendue, montant: montant });
    ...
    var lignes = erreurs.map(function(e){
        ...
        return '<tr' + (reclasse ? ' style="opacity:.6;"' : '') + '>'
            ...
            + '<td>' + (reclasse
                ...
                : '<button class="btn btn-primary" style="padding:4px 10px;font-size:11px;" onclick="syscAppliquerReclassification(\'' + esc(e.compte) + '\', \'' + esc(e.compteRecommande) + '\')">Appliquer la re-classification</button>')
            + '</td></tr>';
    }).join('');
