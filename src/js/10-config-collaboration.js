...
(function(){
    ...
    function myAllowedTabs(){
        ...
        // E3 ne verrouille QUE la liasse (18 onglets séparés, voir
        // pontOngletLiasseVerrouille) — le cycle d'audit (ces 34 onglets-ci) reste
        // pleinement ouvert à un collaborateur affecté, aucune restriction ici.
        if(pontActif) return TABS.map(function(t){ return t.id; });
    ...
    function appliquerVerrouLiassePont(){
        document.querySelectorAll('.liasse-panel').forEach(function(panel){
            var verrouille = panel.classList.contains('active') && pontOngletLiasseVerrouille(panel.id);
            ...
            if(verrouille){
                if(!overlay){
                    ...
                    overlay.innerHTML = '<div style="position:sticky; top:12px; margin:12px auto; max-width:420px; background:#1B2A4A; color:#fff; padding:8px 16px; border-radius:6px; font-size:12px; text-align:center; box-shadow:0 2px 8px rgba(0,0,0,.25);">🔒 Lecture seule — la liasse est réservée à l\'administrateur du cabinet</div>';
