...
(function(){
    ...
    function renderDossiersAffectes(membre, code){
        ...
        lecture.then(function(snap){
            ...
            snap.forEach(function(doc){
                var d = doc.data() || {};
                rows.push({ id: doc.id, intitule: d.intitule || doc.id, client: d.clientAudite || '', exercice: d.exercice || '', statut: d.statut || '' });
            });
            ...
            contentEl.innerHTML = '<p style="font-size:11px; color:#888; margin-bottom:10px;">' + rows.length + ' dossier(s) accessible(s).</p>'
                + rows.map(function(r){
                    return '<div style="border:1px solid #eee; border-radius:6px; padding:8px 10px; margin-bottom:6px; text-align:left;">'
    ...
    function applyAccessControl(){
        ...
        document.getElementById('collab-admin-btn').style.display = isAdmin ? 'inline-block' : 'none';
        document.getElementById('collab-invite-btn').style.display = isAdmin ? 'inline-block' : 'none';
        document.getElementById('collab-password-btn').style.display = isAdmin ? 'inline-block' : 'none';
        ...
        document.getElementById('collab-who').textContent = avatarFor(permData, myName, mySafeKey) + ' ' + myName + (isAdmin ? ' (Administrateur)' : '');
