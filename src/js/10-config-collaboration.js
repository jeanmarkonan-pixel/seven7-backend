...
var TABS = [
    ...
    {id:'questionnaire', label:'✓ Risque du CI', phase:1},
    {id:'risque-inherent', label:'🎯 Risque Inhérent', phase:1},
...
(function(){
    ...
    function renderNouveauModeleCabinet(membre, code){
        ...
        el.innerHTML = '<div style="background:#F5EFE2; border:1px solid #B8975A; border-radius:6px; padding:10px 12px; margin-bottom:12px; text-align:left;">' +
            ...
            '<span id="cabinet-dash-palier-badge" style="display:none; background:#1B2A4A; color:#fff; font-size:10.5px; font-weight:700; padding:2px 9px; border-radius:10px; white-space:nowrap;">Chargement…</span>' +
            '</div>' +
            ...
            boutonEquipe +
            '<button type="button" class="btn" style="width:100%; margin-top:8px; font-size:12px;" onclick="cabinetOuvrirComparaisonPaliers()">💼 Comparer les paliers</button>' +
            '</div>';
        db.collection('cabinets').doc(code).get().then(function(cabDoc){
            trackReads(1);
            var quotaEl = document.getElementById('cabinet-dash-quota-nouveau');
            var badgeEl = document.getElementById('cabinet-dash-palier-badge');
            ...
            if(badgeEl){
                // Libellé lisible ("Pro") plutôt que l'identifiant brut ("PRO") —
                // lu depuis /plans, jamais codé en dur (E5). Le badge affiche
                // d'abord l'identifiant brut, remplacé par le libellé dès que
                // /plans répond ; en cas d'échec, l'identifiant brut reste un
                // repli tout à fait lisible.
                badgeEl.textContent = '💼 ' + planId;
                badgeEl.style.display = 'inline-block';
                db.collection('plans').doc(planId).get().then(function(planDoc){
                    trackReads(1);
                    if(planDoc.exists && planDoc.data().libelle) badgeEl.textContent = '💼 ' + planDoc.data().libelle;
                }).catch(function(){});
            }
    ...
    var cabinetEquipeQuotaAdmins = 9999; // plafond d'admins du cabinet ouvert dans l'écran Équipe
    ...
    function cabinetRafraichirEquipe(){
        ...
        Promise.all([
            ...
            db.collection('cabinets').doc(code).get(),
        ]).then(function(res){
            var membresSnap = res[0], dossiersSnap = res[1], cabinetDoc = res[2];
            trackReads(membresSnap.size + dossiersSnap.size + 1);
            // quotaAdmins : plafond métier d'administrateurs pour ce cabinet (posé à
            // la création — creer-cabinet.mjs). Absent sur les cabinets plus anciens
            // => illimité. Appliqué ici seulement : les règles Firestore ne comptent
            // pas les documents d'une sous-collection.
            var cabinetData = cabinetDoc.exists ? (cabinetDoc.data() || {}) : {};
            cabinetEquipeQuotaAdmins = (typeof cabinetData.quotaAdmins === 'number' && cabinetData.quotaAdmins > 0)
                ? cabinetData.quotaAdmins : 9999;
            var dossiers = [];
            ...
            var nbAdmins = membres.filter(function(m){ return m.role === 'ADMIN'; }).length;
            var plafondAdminsAtteint = nbAdmins >= cabinetEquipeQuotaAdmins;
            if(cabinetEquipeQuotaAdmins < 9999){
                var infoEl = document.getElementById('cabinet-equipe-quota-admins');
                if(infoEl) infoEl.textContent = 'Administrateurs : ' + nbAdmins + ' / ' + cabinetEquipeQuotaAdmins
                    + (plafondAdminsAtteint ? ' — plafond atteint' : '');
            }
            listeEl.innerHTML = membres.map(function(m){
                ...
                // Le plafond quotaAdmins bloque la promotion (jamais la rétrogradation).
                var roleBtn;
                if(soi){
                    roleBtn = '';
                }else if(m.role !== 'ADMIN' && plafondAdminsAtteint){
                    roleBtn = '<button type="button" class="btn" disabled title="Plafond d\'administrateurs atteint pour ce cabinet (' + cabinetEquipeQuotaAdmins + ')." '
                        + 'style="font-size:11px; padding:3px 8px; margin-left:6px; opacity:.5; cursor:not-allowed;">Promouvoir administrateur</button>';
                }else{
                    roleBtn = '<button type="button" class="btn" style="font-size:11px; padding:3px 8px; margin-left:6px;" '
    ...
    window.cabinetChangerRoleMembre = function(uid, nouveauRole){
        ...
        // Plafond d'administrateurs : re-vérifié en base au moment de l'action
        // (le bouton pouvait être affiché avant qu'un autre admin ne promeuve
        // quelqu'un). Ne concerne que la PROMOTION — rétrograder reste possible.
        var garde = (nouveauRole === 'ADMIN' && cabinetEquipeQuotaAdmins < 9999)
            ? db.collection('cabinets').doc(cabinetEquipeCode).collection('membres').where('role', '==', 'ADMIN').get()
                .then(function(snap){
                    trackReads(Math.max(1, snap.size));
                    if(snap.size >= cabinetEquipeQuotaAdmins){
                        throw new Error('Plafond d\'administrateurs atteint pour ce cabinet ('
                            + cabinetEquipeQuotaAdmins + '). Rétrogradez d\'abord un administrateur.');
                    }
                })
            : Promise.resolve();

        garde.then(function(){
            return db.collection('cabinets').doc(cabinetEquipeCode).collection('membres').doc(uid)
