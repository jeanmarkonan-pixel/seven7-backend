...
/* ---------- Installation ---------- */
// Accordéon (22/08, v2) : neuf tableaux affichés à la suite sans repli
// rendaient la page bien trop longue — au premier coup d'œil, ça donnait
// l'impression que « rien ne s'affichait ». Chaque impôt/taxe est maintenant
// une section repliée par défaut, listée d'un coup d'œil, dépliée à la
// demande — les fonctions de calcul tournent normalement même repliées
// (display:none n'empêche pas le JS de lire/écrire les valeurs).
function tfSection(titre, contenuHtml, id){
    return '<div class="tf-section" style="margin-top:10px; border:1px solid #e2e8f0; border-radius:6px; overflow:hidden;">'
        + '<button type="button" class="tf-section-entete" onclick="tfBasculerSection(this)" '
        + 'style="width:100%; text-align:left; padding:12px 16px; background:#eaf2f8; border:none; '
        + 'font-weight:700; font-size:13px; color:#1a5276; cursor:pointer; display:flex; align-items:center; gap:8px;">'
        + '<span class="tf-section-chevron" style="font-size:10px;">▸</span> ' + esc(titre) + '</button>'
        + '<div class="tf-section-contenu" id="' + id + '" style="display:none; padding:14px 16px;">' + contenuHtml + '</div>'
        + '</div>';
}
function tfBasculerSection(btn){
    var contenu = btn.parentNode.querySelector('.tf-section-contenu');
    var chevron = btn.querySelector('.tf-section-chevron');
...
function tfInstaller(){
    ...
    + 'ci-dessus. Cliquez sur un impôt pour déplier son tableau. Chaque tableau porte un pied Total / Solde Initial / '
    + 'Règlement / Solde Final qui suit la dette fiscale comme un compte. Quand le compte SYSCOHADA associé est connu '
    + '(TVA, ITS/CE/TA/TFPC, Patente, CNPS), le Solde Initial est extrait automatiquement de la balance d\'ouverture, '
    + 'et une ligne Comptabilité/Écart compare le déclaré au comptabilisé (alerte 🟠 si écart ≠ 0).</div>'
    + tfSection('TVA', tfRendreTVA()
        ...
        + tfRendreCreditTVA()
        + '</div>', 'tf-sec-tva')
    + tfSection('ITS(447)', tfRendreTableGenerique('impots_groupe_1'), 'tf-sec-groupe1')
    + tfSection('ITS(6413,6414,6415)', tfRendreITSAnnuel(), 'tf-sec-its-annuel')
    + tfSection('Autres Impôts Mensuels', tfRendreTableGenerique('impots_groupe_2'), 'tf-sec-groupe2')
    + tfSection('PATENTE(64;44)', tfRendrePatente(), 'tf-sec-patente')
    + tfSection('CNPS', tfRendreTableGenerique('cnps'), 'tf-sec-cnps')
    + tfSection('CMU', tfRendreTableGenerique('cmu_isolee'), 'tf-sec-cmu')
    + tfSection('IRVM', '<div class="alert alert-info" style="font-size:12px;">Retenue trimestrielle sur dividendes/intérêts '
        + 'distribués — à rapprocher du compte 447 sur la Balance Générale N.</div>' + tfRendreIRVM(), 'tf-sec-irvm')
    + tfSection('BIC', '<div class="alert alert-info" style="font-size:12px;">Deux acomptes provisionnels puis solde de liquidation '
        + '— à rapprocher du compte 4494 sur la Balance Générale N.</div>' + tfRendreBIC(), 'tf-sec-bic');
